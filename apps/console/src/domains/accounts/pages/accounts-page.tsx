...
export function AccountsPage() {
  ...
  }>({ name: "", website: "", industry: "", customFields: {} });
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setOpen(false);
      setForm({ name: "", website: "", industry: "", customFields: {} });
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage customer accounts, people, opportunities, and activity.
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {data.accounts.map((account) => (
            <Link
              key={account.id}
              to={`/dashboard/crm/companies/${account.id}`}
            >
              <Card className="h-full cursor-pointer transition-shadow hover:shadow-lg">
                ...
                <CardContent className="grid grid-cols-3 gap-3 text-center text-sm">
                  <div>
                    <p className="font-semibold">{account.contactCount}</p>
                    <p className="text-xs text-muted-foreground">Contacts</p>
                  </div>
                  <div>
                    <p className="font-semibold">{account.opportunityCount}</p>
                    <p className="text-xs text-muted-foreground">Deals</p>
                  </div>
                  <div>
                    <p className="font-semibold">
                      {account.pipelineValue.toLocaleString()}
                    </p>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Create a shared profile for contacts and opportunities.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={submit} className="space-y-4">
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={createAccount.isPending}>
                {createAccount.isPending ? "Creating…" : "Create company"}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
