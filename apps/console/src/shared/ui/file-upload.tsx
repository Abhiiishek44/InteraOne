...
export function FileUpload({
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const presignedResponse = await storageApi.generatePresignedUploadUrl(
        file.name,
        file.type,
      ...
      const downloadResponse = await storageApi.generatePresignedDownloadUrl(
        fileKey,
        downloadUrlExpiry
      );
...
export function useFileUpload() {
  ...
  const deleteFile = async (fileKey: string): Promise<boolean> => {
    try {
      await storageApi.deleteStorageFile(fileKey);
